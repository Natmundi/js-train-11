...
function setToArray(mySet) {
  ...
  const filteredArr = arr.filter((elem) => typeof elem === "string");
...
function iterateSet(set) {
  ...
  const valuSet = set.values();
  for (const value of valuSet) {
    console.log(value);
  }
  const entrSet = set.entries();
  for (const entri of entrSet) {
    console.log(entri);
  }
}
